...
/* holds the processes that is ready */
static struct list readyQueue;
...
/* Statistics. */
static long long kernelTicks;  /* holds the number of ticks in kernel threads. */
static long long idleTicks;    /* holds the number of ticks while in idle */
static long long numTicks;    /* holds the number of timer ticks in user programs. */
...
// outputs the thread state
void
thread_print_stats (void) 
{
  printf ("Thread: %lld idle ticks, %lld kernel ticks, %lld user ticks\n",
          idleTicks, kernelTicks, numTicks);
}
...
thread_unblock (struct thread *t) 
{
  ...
  list_insert_ordered(&readyQueue, &t->elem, (list_less_func *) &cmp_priority, NULL);
  t->status = THREAD_READY;
  intr_set_level (old_level);
}

// returns the current thread's data member name
const char *
thread_name (void) 
{
...
// returns the current thread's data member tid
tid_t
thread_tid (void) 
{
...
thread_yield (void) 
{
  ...
  if (cur != idle_thread)
  {
    list_insert_ordered(&readyQueue, &cur->elem,(list_less_func *) &cmp_priority,NULL);
  }
...
// Returns the priorrity of the current thread
int
...
{
  enum intr_level old_level = intr_disable ();
  int loadAv = fp_to_int_round( mult_mixed(load_avg, 100) );
  intr_set_level (old_level);
  return loadAv;
}
...
{
  enum intr_level old_level = intr_disable ();
  int recentCPU = fp_to_int_round( mult_mixed(thread_current()->recent_cpu, 100) );
  intr_set_level (old_level);
  return recentCPU;
}
...
next_thread_to_run (void) 
{
  ...
  {
    return list_entry (list_pop_front (&readyQueue), struct thread, elem);
  }
}
...
void test_max_priority (void)
{
  ...
  struct thread *t = list_entry(list_front(&readyQueue),
				struct thread, elem);
...
{
  int term2 = list_size(&readyQueue);
