...
{
  enum intr_level old_level;
  // holds a boolean value that determines if its a success or not
  bool success;

  // checks sema that it is initialized
  ASSERT (sema != NULL);

  // disable interrupt and save state to old_level
  old_level = intr_disable ();
  // if sema value is greater than zero then decrement value and return true
  if (sema->value > 0) 
  {
    --(sema->value);
    success = true; 
  }
  // if value is less than zero then just return false
  else
  {
    success = false;
  }
